  ...
  <div class="rounded-lg card-base">
    <div class="space-y-2">
      <div :class="top && 'flex justify-between space-x-2'">
        ...
        >
          <span class="text-black/50 dark:text-white/30">{{ username }}/</span
          ><span>{{ name }}</span>
        </h3>
    ...
    <div class="mt-4">
      ...
      >
        <span>Language:</span>
        <Icon
          v-if="icons[language as keyof typeof icons]"
          :name="icons[language as keyof typeof icons]"
          class="h-5 w-5"
        />
        <span v-else class="text-sm">{{ language }}</span>
      </div>
